...
/**
 * [P3-2] SDVC 엔진과의 대화 API 뼈대.
 *
 * 이 라우트가 책임지는 것은 네 가지뿐이다: 로그인 확인 → 입력 검증 →
 * 서버 키 확인 → Claude 스트림 전달. 진행대본(5블록 7단계) 프롬프트는
 * [P3-3]에서, 대화 내용의 DB 저장은 [P3-4]에서 얹는다.
 */
...
export async function POST(request: Request) {
  ...
  const stream = await createChatStream({ apiKey, messages });
