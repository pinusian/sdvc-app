...
const SITE_ACCOUNTS_GUIDANCE_LINES: string[] = [
  ...
  '- 로그아웃: `POST` `siteApi("/auth/logout")` (본문 없음) → `{ok: true}`, 로그인 쿠키가 지워진다.',
  "  로그인 화면에는 **반드시 로그아웃 버튼도 함께** 낸다 — 빠뜨리면 사용자가 다른 계정으로",
  "  써보려 해도 방법이 없다.",
  '- 비밀번호 변경: `POST` `siteApi("/auth/change-password")` body `{currentPassword, newPassword}`',
  "  → `{ok: true}`. 개발자가 방문자 관리 화면에서 대신 만들어준 **임시 비밀번호**로 로그인한",
  "  사용자가 자기 것으로 바꿀 때 쓴다 — 로그인 뒤 화면(설정 등)에 이 기능을 반드시 넣는다,",
  "  안 넣으면 그 임시 비밀번호가 사실상 영구 비밀번호로 남는다. 현재 비밀번호가 틀리면",
  '  `{error: "현재 비밀번호가 올바르지 않습니다."}`가 온다.',
  "",
...
/**
 * [BL-028] 명세 충돌 감지 + 범위 고정 — 구현(5)·유지보수(6) 공통.
 *
 * 2026-09-18 실사용자 사고: 명세에 "누가 사용하나요? — 혼자 쓰는 개인용
 * 웹앱"이라고 적혀 있었고 **그 문답이 같은 대화 기록 안에 있었는데도**,
 * "로그인 기능 추가" 요청이 아무 경고 없이 실행돼 다중 사용자 앱이 됐다.
 * 동시에 로그인과 무관한 AI 기능 5개(요약·추천·토론주제·토론·담론)의
 * 호출 방식과 저장 위치까지 함께 바뀌어, 사용자가 쌓아둔 것이 전부
 * 화면에서 사라졌다.
 *
 * 대화 기록은 잘리지 않고 통째로 전달되므로(`listMessages`) 모델은 명세를
 * 언제나 볼 수 있다 — 못 봐서가 아니라 **보라고 시키지 않아서** 생긴 사고다.
 */
const SCOPE_GUARD_LINES: string[] = [
  "**손대기 전에 명세와 맞는지 먼저 확인한다(중요).** 이 대화 앞부분에는 이 프로젝트를",
  "처음 만들 때 정한 명세(무엇을·누구를 위해·어디까지)가 남아 있다. 새 요청이 그것과",
  "**어긋나면 작업을 시작하지 않고 멈춘다.** 원래 명세의 해당 문구를 그대로 인용해 보여주고,",
  '"원래는 이렇게 정하셨는데 이번 요청과 다릅니다. 명세를 바꾸실 건가요, 아니면 명세에',
  '맞는 다른 방법을 찾을까요?"라고 묻는다. 사용자가 답한 뒤에 진행한다.',
  "",
  "**요청받지 않은 것은 함께 바꾸지 않는다(중요).** 고칠 것이 더 보여도 이번 요청의 범위",
  '밖이면 손대지 말고 **말로만 알린다**("이것도 같이 고치는 게 좋아 보입니다 — 할까요?").',
  "특히 이번 요청과 **무관한 기능**의 동작 방식·저장 방식을 함께 바꾸지 않는다. 기능 하나를",
  "고치는 일이 다른 기능을 건드리면 사용자는 무엇이 왜 달라졌는지 알 수 없게 된다.",
];

/**
 * [BL-029] 데이터 이주 규칙 — 구현(5)·유지보수(6) 공통.
 *
 * 같은 사고에서 기록이 `localStorage.reading_records_v1` → 서버로,
 * AI 산출물이 `*_v1` → `*_v2`로 조용히 옮겨졌다. 데이터는 지워지지
 * 않았지만 앱이 그 자리를 더 이상 읽지 않아 전부 사라져 보였다 —
 * 삭제보다 나쁘다(원인 파악조차 어렵다). [P7-12] "안 되는 것은 조용히
 * 넘기지 않는다"와 같은 원칙을 데이터에도 적용한다.
 */
const DATA_MIGRATION_LINES: string[] = [
  "**데이터가 있던 자리를 옮길 때(중요)**: 저장 위치(`localStorage` 키 이름, 로컬↔서버,",
  "자료 구조)를 바꾸는 변경은 **기존 데이터를 옮기는 코드를 함께 낸다.** 옮길 수 없으면",
  '반드시 먼저 이렇게 알린다: "이렇게 바꾸면 그동안 쌓인 기존 데이터가 화면에 보이지 않게 됩니다."',
  "키 이름에 `_v2` 같은 버전 번호를 새로 붙이는 변경이 특히 위험하다 — 코드는 멀쩡해 보이는데",
  "사용자가 쌓아온 것만 조용히 사라진다. 지워지지 않았어도 **보이지 않으면 잃은 것이다.**",
];
...
export const SDVC_BLOCKS: SdvcBlock[] = [
  ...
  },
  {
    ...
    instruction: [
      ...
      "승인 게이트도 없다 — 고쳐달라는 말이 곧 승인이다. **다만** 그 승인은 **요청한 그것**에",
      "대한 승인일 뿐이다: 명세와 어긋나는 변경, 요청 범위 밖의 변경, 기존 데이터가 사라지는",
      "변경은 위에 적은 대로 **먼저 알리고 답을 받은 뒤에** 한다.",
      "",
      ...DATA_MIGRATION_LINES,
      "",
      ...NO_EXECUTION_LINES,
